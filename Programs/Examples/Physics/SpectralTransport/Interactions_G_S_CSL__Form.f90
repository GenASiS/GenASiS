module Interactions_G_S_CSL__Form

  !-- Interactions_Generic_Spectral_ChartSingleLevel_Form

  use GenASiS
  use Interactions_G_S__Form

  implicit none
  private

  type, public, extends ( Interactions_CSL_Template ) :: &
    Interactions_G_S_CSL_Form
  contains
    procedure, public, pass :: &
      Initialize
    procedure, public, pass :: &
      AllocateField
  end type Interactions_G_S_CSL_Form
...
contains
  ...
  subroutine Initialize &
    ...
    class ( Interactions_G_S_CSL_Form ), intent ( inout ) :: &
      IC
    ...
    if ( IC % Type == '' ) &
      IC % Type = 'an Interactions_G_S_CSL'
  ...
  subroutine AllocateField ( IC )
    ...
    case ( 'GENERIC_SPECTRAL' )
      allocate ( Interactions_G_S_Form :: IC % Field )
    case default
      call Show ( 'InteractionsType not recognized', CONSOLE % ERROR )
      call Show ( IC % InteractionsType, 'InteractionsType', &
                  CONSOLE % ERROR )
      call Show ( 'Interactions_G_S_CSL__Form', 'module', CONSOLE % ERROR )
...
end module Interactions_G_S_CSL__Form
