module Slope_RM_I_I_J__Form

  !-- Slope_RadiationMoments_Interactions_Implicit__Form
  ...
  type, public, extends ( Slope_H_Form ) :: Slope_RM_I_I_J_Form
    integer ( KDI ) :: &
      iEnergy_B
  ...
  contains
    procedure, private, pass :: &
      InitializeAllocate_RM_I_I_J
    generic, public :: &
      Initialize => InitializeAllocate_RM_I_I_J
    procedure, public, pass :: &
      Compute
    final :: &
      Finalize
  end type Slope_RM_I_I_J_Form
...
contains
  ...
  subroutine InitializeAllocate_RM_I_I_J ( S, R )

    class ( Slope_RM_I_I_J_Form ), intent ( inout ) :: &
      S
    ...
    if ( S % Type  ==  '' ) &
      S % Type  =  'a Slope_RM_I_I_J' 
    
    Name  =  trim ( R % Name ) // '_Slp_RM_I_I_J'
...
!    end select
  ...
  end subroutine InitializeAllocate_RM_I_I_J
  ...
  subroutine Compute ( S, dT, T_Option )
    ...
    if ( .not. associated ( S % Interactions ) ) then
      call Show ( 'Please set Interactions', &
                  CONSOLE % ERROR )
      call Show ( 'Slope_RM_I_I_J__Form', 'module', CONSOLE % ERROR )
  ...
  impure elemental subroutine Finalize ( S )
    ...
    type ( Slope_RM_I_I_J_Form ), intent ( inout ) :: &
      S
...
end module Slope_RM_I_I_J__Form
