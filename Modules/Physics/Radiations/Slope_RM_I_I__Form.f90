module Slope_RM_I_I__Form
  ...
  type, public, extends ( Slope_H_Form ) :: Slope_RM_I_I_Form
    integer ( KDI ) :: &
      iEnergy_B
...
!      Slope_DFV => null ( )
  ...
  contains
    procedure, private, pass :: &
      InitializeAllocate_RM_I_I
    generic, public :: &
      Initialize => InitializeAllocate_RM_I_I
    procedure, public, pass :: &
      Compute
    final :: &
      Finalize
  end type Slope_RM_I_I_Form
...
contains
  ...
  subroutine InitializeAllocate_RM_I_I ( S, R )

    class ( Slope_RM_I_I_Form ), intent ( inout ) :: &
      S
    ...
    if ( S % Type  ==  '' ) &
      S % Type  =  'a Slope_RM_I_I' 
    
    Name  =  trim ( R % Name ) // '_Slp_RM_I_I'
...
!    end select
  ...
  end subroutine InitializeAllocate_RM_I_I
  ...
  subroutine Compute ( S, dT, T_Option )
    ...
    if ( .not. associated ( S % Interactions ) ) then
      call Show ( 'Please set Interactions', &
                  CONSOLE % ERROR )
      call Show ( 'Slope_RM_I_I__Form', 'module', CONSOLE % ERROR )
...
!        end if
  ...
  impure elemental subroutine Finalize ( S )
    ...
    type ( Slope_RM_I_I_Form ), intent ( inout ) :: &
      S
