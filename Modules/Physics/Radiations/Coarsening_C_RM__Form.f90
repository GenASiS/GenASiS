module Coarsening_C_RM__Form

  !-- Coarsening_Central_RadiationMoments_Form
  ...
  type, public, extends ( Coarsening_C_Form ) :: Coarsening_C_RM_Form
    integer ( KDI ) :: &
      nRadialZero, &
      nPolarZero
    real ( KDR ) :: &
...
contains
  ...
  subroutine Initialize_C_RM &
               ( C, RM, G, RadiusZeroOption, nRadialZeroOption, &
                 nPolarZeroOption )

    class ( Coarsening_C_RM_Form ), intent ( inout ) :: &
      C
    ...
    if ( C % Type == '' ) &
      C % Type  =  'a Coarsening_C_RM' 
    
    call C % Coarsening_C_Form % Initialize ( G )

    C % RadiationMoments  =>  RM
    ...
    class default
      ...
      call Show ( 'Coarsening_C_RM__Form', 'module', CONSOLE % ERROR )
      call Show ( 'Initialize_C_F', 'subroutine', CONSOLE % ERROR )
      call PROGRAM_HEADER % Abort ( )
    end select !-- A

  end subroutine Initialize_C_RM


  subroutine Show_FS ( FS )

    class ( Coarsening_C_RM_Form ), intent ( in ) :: &
      FS
  ...
  subroutine Compute ( FS, C )
    ...
    select type ( A  =>  FS % Atlas )
      class is ( Atlas_SCG_C_Form )
    associate &
      ( RM      =>  C % RadiationMoments, &
        G       =>  C % Geometry, &
        C_GS_C  =>  A % Chart_GS_C )

    call Search &
           ( RM % iaBalanced, RM % MOMENTUM_DENSITY_B_D_2, iMomentum_2 )
    call Search &
           ( RM % iaBalanced, RM % MOMENTUM_DENSITY_B_D_3, iMomentum_3 )
  ...
  impure elemental subroutine Finalize ( C )
    ...
    type ( Coarsening_C_RM_Form ), intent ( inout ) :: &
      C

    nullify ( C % RadiationMoments )

  end subroutine Finalize

  
end module Coarsening_C_RM__Form
