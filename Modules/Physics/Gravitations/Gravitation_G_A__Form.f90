module Gravitation_G_A__Form
  ...
  type, public, extends ( Geometry_F_A_Form ) :: Gravitation_G_A_Form
  contains
    procedure, private, pass :: &
      InitializeAllocate_FS
    final :: &
      Finalize
  end type Gravitation_G_A_Form
...
contains
  ...
  subroutine InitializeAllocate_FS &
    ...
    class ( Gravitation_G_A_Form ), intent ( inout ), target :: &
      FSA
    ...
    if ( .not. PreviouslyAllocated ) then
      do iC  =  1,  nC
        ...
        allocate ( Gravitation_G_C_Form :: FSA % FieldSet_C ( iC ) % Element ) 
  ...
  impure elemental subroutine Finalize ( GA )
    ...
    type ( Gravitation_G_A_Form ), intent ( inout ) :: &
      GA

  end subroutine Finalize


end module Gravitation_G_A__Form
