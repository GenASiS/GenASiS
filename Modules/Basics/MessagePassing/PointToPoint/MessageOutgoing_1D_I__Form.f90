!-- MessageOutgoing_1D_I_Form provides the concrete extension of 
!   Message_1D_Template for integer datatype to handle sending array of 
!   messages.
...
module MessageOutgoing_1D_I__Form
  ...
  type, public, extends ( Message_1D_Template ) :: MessageOutgoing_1D_I_Form
    type ( MessageOutgoing_I_Form ), dimension ( : ), allocatable :: &
      Message
  contains
    ...
    final :: &
      Finalize
  end type MessageOutgoing_1D_I_Form
...
contains
  ...
  subroutine InitializeAllocate ( M_1D, C, Tag, Rank, nElements )
    ...
    class ( MessageOutgoing_1D_I_Form ), intent ( inout ), target :: &
      M_1D
    ...
    integer ( KDI ) :: &
      iM    !-- iMessage
    type ( MessageOutgoing_I_Form ), pointer :: &
      M
  ...
  subroutine SendAll ( M_1D )
    ...
    M => M_1D % Message
    select type ( M )
    type is ( MessageOutgoing_I_Form )
      do iM = 1, M_1D % nMessages
        call M ( iM ) % Send ( )
      end do
    end select
  ...
  elemental subroutine Finalize ( M_1D )
    ...
    type ( MessageOutgoing_1D_I_Form ), intent ( inout ) :: &
      M_1D
...
end module MessageOutgoing_1D_I__Form
