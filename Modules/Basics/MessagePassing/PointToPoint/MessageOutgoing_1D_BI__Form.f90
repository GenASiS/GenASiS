!-- MessageOutgoing_1D_BI_Form provides the concrete extension of 
!   Message_1D_Template for big (long) integer datatype to handle sending 
!   array of messages.
...
module MessageOutgoing_1D_BI__Form
  ...
  type, public, extends ( Message_1D_Template ) :: MessageOutgoing_1D_BI_Form
    type ( MessageOutgoing_BI_Form ), dimension ( : ), allocatable :: &
      Message
 contains
    ...
    final :: &
      Finalize
  end type MessageOutgoing_1D_BI_Form
...
contains
  ...
  subroutine InitializeAllocate ( M_1D, C, Tag, Rank, nElements )
    ...
    class ( MessageOutgoing_1D_BI_Form ), intent ( inout ), target :: &
      M_1D
    ...
    integer ( KDI ) :: &
      iM    !-- iMessage
    type ( MessageOutgoing_BI_Form ), pointer :: &
      M
  ...
  subroutine SendAll ( M_1D )
    ...
    M => M_1D % Message
    select type ( M )
    type is ( MessageOutgoing_BI_Form )
      do iM = 1, M_1D % nMessages
        call M ( iM ) % Send ( )
      end do
    end select
  ...
  elemental subroutine Finalize ( M_1D )
    ...
    type ( MessageOutgoing_1D_BI_Form ), intent ( inout ) :: &
      M_1D
...
end module MessageOutgoing_1D_BI__Form
