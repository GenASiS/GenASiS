!-- MessageIncoming_1D_BI_Form provides the concrete extension of 
!   Message_1D_Template for big ( long )integer datatype to handle receiving 
!   array of messages.
...
module MessageIncoming_1D_BI__Form
  ...
  type, public, extends ( Message_1D_Template ) :: MessageIncoming_1D_BI_Form
    type ( MessageIncoming_BI_Form ), dimension ( : ), allocatable :: &
      Message
  contains
    ...
    final :: &
      Finalize
  end type MessageIncoming_1D_BI_Form
...
contains
  ...
  subroutine InitializeAllocate ( M_1D, C, Tag, Rank, nElements )
    ...
    class ( MessageIncoming_1D_BI_Form ), intent ( inout ), target :: &
      M_1D
    ...
    integer ( KDI ) :: &
      iM    !-- iMessage
    type ( MessageIncoming_BI_Form ), pointer :: &
      M      
  ...
  subroutine Receive ( M_1D )
    ...
    M => M_1D % Message 
    select type ( M )
    type is ( MessageIncoming_BI_Form )
      do iM = 1, M_1D % nMessages
        call M ( iM ) % Receive ( )
      end do
    end select
  ...
  elemental subroutine Finalize ( M_1D )
    ...
    type ( MessageIncoming_1D_BI_Form ), intent ( inout ) :: &
      M_1D
...
end module MessageIncoming_1D_BI__Form
